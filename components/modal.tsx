...
export default function Modal({ onClose }: { onClose: () => void }) {
  ...
  const [loadedVideo, toggleLoaded] = useState(false);
  ...
  return (
    <div tabIndex={0} ref={modal} className="fixed top-0 z-50 flex h-full w-full items-center justify-center bg-edgeset/75" onKeyDown={(e) => {
      if (e.key === 'Escape') {
        onClose();
      }
    }}>
      ...
      {!loadedVideo ? <div role="status" className="absolute top-[50%]">
